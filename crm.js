...
function setView(view){
  const analytics=view==='analytics',dynamics=view==='dynamics',funnel=view==='funnel';
  ...
  $('#emptyState').classList.toggle('hidden',funnel||dynamics||data.length>0);
...
function renderOverview(rows){const active=rows.filter(x=>x.active),live=active.filter(x=>x.age!=null&&x.age<=30),stale=active.filter(x=>x.age!=null&&x.age>90),task=rows.filter(x=>x.task);$('#kTotal').textContent=fmt(rows.length);$('#kActive').textContent=fmt(active.length);$('#kLive').textContent=fmt(live.length);$('#kStale').textContent=fmt(stale.length);$('#kTasks').textContent=fmt(task.length);bars('#statusBars',count(rows,'status'));bars('#verticalBars',count(rows,'vertical'));const owners=count(rows,'owner');$('#owners').innerHTML=Object.entries(owners).sort((x,y)=>y[1]-x[1]).map(([o,n])=>`<tr><td>${o}</td><td class="num"><b>${fmt(n)}</b></td><td class="num">${fmt(rows.filter(x=>x.owner===o&&x.age!=null&&x.age<=30).length)}</td><td class="num">${fmt(rows.filter(x=>x.owner===o&&x.age!=null&&x.age>90).length)}</td><td class="num">${fmt(rows.filter(x=>x.owner===o&&x.task).length)}</td></tr>`).join('');const q=[['Компания',x=>x.company&&x.company!=='Без названия'],['Вертикаль',x=>x.vertical!=='Не указана'],['Телефон',x=>x.phone],['Email',x=>x.email],['ИНН',x=>x.inn],['Автопарк',x=>x.fleet!=null],['Кол-во ATOM',x=>x.atom!=null],['След. задача',x=>x.task]];$('#quality').innerHTML=q.map(([n,f])=>{const p=rows.length?Math.round(rows.filter(f).length/rows.length*100):0;return`<div class="qrow"><div>${n}</div><div class="qbar"><span style="width:${p}%"></span></div><div class="qval">${p}%</div></div>`}).join('')}
function summary(){const activeStatuses=[...new Set(data.filter(x=>x.active).map(x=>x.status))].sort(),allStatuses=[...new Set(data.filter(x=>x.active).map(x=>x.status))].sort(),statusOptions=[...allStatuses,'Неактивные / дисквалифицированные'],verticalOptions=[...new Set(data.map(x=>x.vertical))].sort(),defaultVerticals=verticalOptions.filter(v=>['GR / B2G','Каршеринг','Корпоративные','Не указана'].includes(v));setupMulti('#fStatus',statusOptions,statusOptions);setupMulti('#fVertical',verticalOptions,defaultVerticals);setupMulti('#fOwner',[...new Set(data.map(x=>x.owner))].sort(),[]);setupMulti('#fHealth',['≤30 дней','31-90 дней','>90 дней','Нет даты','Неактивный'],[]);apply()}
