...
(()=>{const U='https://enlyiedwkarajvfsilel.supabase.co',K='sb_publishable_YK0GMEpWNTnEp3ImIvONKQ_3IPZdvq5',BUCKET='plan-source-files',gate=document.getElementById('authGate'),shell=document.getElementById('appShell'),st=document.getElementById('authStatus'),mail=document.getElementById('authEmail'),send=document.getElementById('authSend'),user=document.getElementById('authUserEmail'),alfaFile=document.getElementById('alfaFile'),alfaDate=document.getElementById('alfaActualDate'),alfaStatus=document.getElementById('alfaFileStatus');let sb=window.ATOMSharedSupabase||null;try{if(!sb)sb=window.supabase?.createClient(U,K,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true,flowType:'implicit',storageKey:'atom-sales-plan-auth'}})}catch{}const SHARED_AUTH_KEY='atom-global-auth-v1';const readSharedAuth=()=>{try{return JSON.parse(localStorage.getItem(SHARED_AUTH_KEY)||'{}')}catch{return {}}};const saveSharedAuth=(access,refresh)=>{const prev=readSharedAuth();localStorage.setItem(SHARED_AUTH_KEY,JSON.stringify({access_token:access||prev.access_token||'',refresh_token:refresh||prev.refresh_token||'',updated_at:Date.now()}))};const clearSharedAuth=()=>localStorage.removeItem(SHARED_AUTH_KEY);const ok=e=>/^[^@\s]+@atom\.team$/i.test(e||'');const enc=t=>{const b=new TextEncoder().encode(String(t||''));let s='';b.forEach(x=>s+=String.fromCharCode(x));return btoa(s).replace(/\+/g,'-').replace(/\//g,'_').replace(/=+$/,'')};const safe=n=>String(n||'file').replace(/[\\/]+/g,'_').replace(/[\u0000-\u001f\u007f]/g,'_').slice(0,160);const setAlfa=(t,cl='')=>{if(!alfaStatus)return;alfaStatus.textContent=t;alfaStatus.className='statusline'+(cl?' '+cl:'')};const show=e=>{gate.classList.add('app-hidden');shell.classList.remove('app-hidden');user.textContent=e};async function open(s){if(!s?.access_token)return false;try{const{data,error}=await sb.auth.getUser(s.access_token);if(error||!ok(data.user?.email))return false;localStorage.setItem('atom_access_token',s.access_token);if(s.refresh_token)localStorage.setItem('atom_refresh_token',s.refresh_token);saveSharedAuth(s.access_token,s.refresh_token);show(data.user.email);return true}catch{return false}}async function boot(){const sharedReady=window.ATOM_AUTH_READY?await window.ATOM_AUTH_READY:null;if(sharedReady?.client)sb=sharedReady.client;if(sharedReady?.session?.access_token&&await open(sharedReady.session))return;if(!sb){gate.classList.remove('app-hidden');st.textContent='Не удалось загрузить модуль входа.';return}try{const{data}=await sb.auth.getSession();if(data.session&&await open(data.session))return;const shared=readSharedAuth(),a=localStorage.getItem('atom_access_token')||shared.access_token||'',r=localStorage.getItem('atom_refresh_token')||shared.refresh_token||'';if(a&&r){const x=await sb.auth.setSession({access_token:a,refresh_token:r});if(x.data.session&&await open(x.data.session))return}if(a){const chk=await fetch(U+'/functions/v1/atom-auth',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'check',access_token:a})});if(chk.ok){const d=await chk.json();if(d.allowed){saveSharedAuth(a,r);show(d.email);return}}}if(r){const rr=await fetch(U+'/functions/v1/atom-auth',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'refresh',refresh_token:r})});if(rr.ok){const d=await rr.json();if(d.access_token){localStorage.setItem('atom_access_token',d.access_token);localStorage.setItem('atom_refresh_token',d.refresh_token||r);saveSharedAuth(d.access_token,d.refresh_token||r);const x=await sb.auth.setSession({access_token:d.access_token,refresh_token:d.refresh_token||r});if(x.data.session&&await open(x.data.session))return}}}}catch{}gate.classList.remove('app-hidden');st.textContent='Введите рабочую почту @atom.team. Пароль не нужен.'}send.onclick=async()=>{const e=mail.value.trim().toLowerCase();if(!ok(e)){st.textContent='Доступ разрешен только для @atom.team.';st.className='auth-status bad';return}send.disabled=true;st.textContent='Отправляю ссылку...';try{await window.ATOMAuth.send(e,location.origin+'/production-and-sales-plan/');st.textContent='Ссылка отправлена на '+e+'.';st.className='auth-status ok'}catch(err){st.textContent=err.message;st.className='auth-status bad'}finally{send.disabled=false}};mail.addEventListener('keydown',e=>{if(e.key==='Enter')send.click()});document.getElementById('authLogout').onclick=async()=>{try{await sb.auth.signOut({scope:'local'})}catch{}localStorage.removeItem('atom_access_token');localStorage.removeItem('atom_refresh_token');clearSharedAuth();location.reload()};if(alfaDate){const d=new Date();alfaDate.value=[d.getFullYear(),String(d.getMonth()+1).padStart(2,'0'),String(d.getDate()).padStart(2,'0')].join('-')}if(alfaFile)alfaFile.addEventListener('change',async()=>{
