(()=>{
'use strict';
const PARSER_VERSION='2.5.8';
try{
  if(localStorage.getItem('atom-parser-version')!==PARSER_VERSION){
    localStorage.removeItem('atom-production-sales-plan-current-model-v1');
...
function parseWorkbook(buf){
  ...
  const firstMetric=matcher=>{for(const row of rows){const label=rowLabel(row),s=k(label);if((matcher instanceof RegExp&&matcher.test(s))||(typeof matcher==='function'&&matcher(s)))return{...rowMetric(row),label}}return{found:false,months:{},year:null,yearFound:false,label:''}};
  const metrics={
  ...
  const hasVerticalToken=(s,v)=>new RegExp(`(^|[^a-z0-9])${v.toLowerCase()}([^a-z0-9]|$)`,'i').test(s);
  const isAggregate=(s,v)=>{const t=s.replace(/\s+/g,' '),vl=v.toLowerCase();return t===vl||t===`итого ${vl}`||(hasVerticalToken(t,v)&&(t.includes('контракт')||t.includes('забронирован')||t.startsWith('итого ')))};
  ...
  for(let ri=h.ri+1;ri<rows.length;ri++){
    ...
    if(/^план |^выпуск |^отгрузка |^передано |^контракты/.test(s))continue;
...
function metricRow(label,metric,cls=''){const missing=!metric?.found?' is-missing':'';return `<tr class="${cls}${missing}"><td class="dash-label">${esc(label)}</td><td class="dash-total">${metric?.found?dot(metricYear(metric)):'·'}</td>${monthCells(metric)}</tr>`}
...
function clientSummaryRow(layer,metric,count){return `<tr class="layer-summary"><td class="layer-name"><strong>${esc(layer)}</strong><small>${count?`${count} ${projectWord(count)}`:'итого'}</small></td><td class="project-name"><strong>Итого ${esc(layer)}</strong></td><td class="dash-total">${dot(metricYear(metric))}</td>${monthCells(metric)}</tr>`}
function clientDetailRow(x){const product=x.product?`<small>${esc(x.product)}</small>`:'';return `<tr class="client-detail"><td></td><td class="project-name"><strong>${esc(x.displayName)}</strong>${product}</td><td class="dash-total">${dot(metricYear(x))}</td>${monthCells(x)}</tr>`}
function kpiCard(label,metric,note,tone=''){return `<div class="analytics-kpi ${tone}"><div class="analytics-kpi-label">${esc(label)}</div><div class="analytics-kpi-value">${metric?.found?fmt(metricYear(metric)):'·'}</div><div class="analytics-kpi-note">${esc(note)}</div></div>`}
function renderModel(model,templateName){
  ...
  one.innerHTML=`
    ...
    <div class="analytics-filterbar"><div class="analytics-filter-group"><span class="analytics-filter-label">ПЕРИОД</span><span class="period-chip active">6 мес</span><span class="period-range">Июль - декабрь 2026</span></div><div class="analytics-filter-group source"><span class="analytics-filter-label">ИСТОЧНИК</span><span class="source-pill">${esc(model.sheetName||'S&OP09 plan')}</span></div></div>
    <div class="analytics-kpi-grid">${kpiHtml}</div>
    <section class="analytics-section"><div class="analytics-section-title">БАЛАНС ПРОИЗВОДСТВА И ПРОДАЖ · 6 МЕС</div><div class="analytics-table-wrap"><table class="analytics-table balance-table"><thead><tr><th>Показатель</th><th>Итого 2026</th>${VIEW_MONTHS.map(m=>`<th>${m}</th>`).join('')}</tr></thead><tbody>${balance}</tbody></table></div></section>
    <section class="analytics-section distribution-section"><div class="analytics-section-title">КОММЕРЧЕСКОЕ РАСПРЕДЕЛЕНИЕ ПО СЛОЯМ · 6 МЕС</div><div class="analytics-table-wrap"><table class="analytics-table distribution-table"><thead><tr><th>Бизнес-слой</th><th>Компания / проект</th><th>Итого 2026</th>${VIEW_MONTHS.map(m=>`<th>${m}</th>`).join('')}</tr></thead><tbody>${distribution}</tbody></table></div></section>
...
(()=>{
  ...
  version.textContent=`Версия v2.5.8 · загрузка ${loadedAt}`;
})();
...
if(uploadGrid&&!$('smmtCard')){
  const card=document.createElement('label');card.className='drop-card';card.id='smmtCard';
  ...
  const templateCard=$('templateCard');if(templateCard)uploadGrid.insertBefore(card,templateCard);else uploadGrid.appendChild(card);
}
...
const observer=new MutationObserver(()=>setTimeout(updateCount,0));['salesCard','templateCard','smmtCard'].forEach(id=>{const el=$(id);if(el)observer.observe(el,{attributes:true,attributeFilter:['class']})});
setTimeout(updateCount,0);loadSmmt().then(file=>{if(file)setSmmt(file,true)});
const style=document.createElement('style');style.id='header-smmt-v1';style.textContent=`
...
let saved={};try{saved=JSON.parse(localStorage.getItem(STATE_KEY)||'{}')||{}}catch{saved={}}
...
function detailRows(row){const rows=[];let next=row.nextElementSibling;while(next&&next.classList.contains('client-detail')){rows.push(next);next=next.nextElementSibling}return rows}
function applyState(row,collapsed){const details=detailRows(row);if(!details.length)return;row.classList.toggle('layer-collapsed',collapsed);row.setAttribute('aria-expanded',collapsed?'false':'true');details.forEach(r=>{r.style.display=collapsed?'none':''})}
function bindLayers(){const table=document.querySelector('.distribution-table');if(!table)return;table.querySelectorAll('tr.layer-summary').forEach(row=>{const details=detailRows(row);if(!details.length){row.classList.add('layer-static');return}if(row.dataset.layerBound==='1')return;row.dataset.layerBound='1';row.classList.add('layer-toggle-row');row.tabIndex=0;row.setAttribute('role','button');const key=layerKey(row);applyState(row,Boolean(saved[key]));const toggle=()=>{const collapsed=!row.classList.contains('layer-collapsed');saved[key]=collapsed;applyState(row,collapsed);saveState()};row.addEventListener('click',toggle);row.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();toggle()}})})}
const style=document.createElement('style');style.id='business-layer-toggle-style';style.textContent=`
